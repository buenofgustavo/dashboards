...
fig2 = go.Figure(
    data=[
        ...
        go.Scatter(
            ...
            text=fat_cliente_graf2_formatado,  # Usando os valores como texto
            textposition='top center',  # Posição do texto (no topo do ponto)
            textfont=dict(size=16, color='black'),  # Estilo do texto
...
status_desejados_graf3 = ["CARGA RECUSADA", "CANCELADO - CLIENTE", "DECLINADO - FALTA ATENDIMENTO"]
df_filtrado_graf3 = df[df['STATUS'].isin(status_desejados_graf3)]

# Agrupar por TOMADOR e contar a ocorrência de cada STATUS
contagem_por_tomador_graf3 = df_filtrado_graf3.groupby('TOMADOR')['STATUS'].value_counts().unstack(fill_value=0)
...
fig3.update_layout(
    title='Faturamento Potencial Perdido (TOP 10)',  # Título do gráfico
    title_font_size=20,  # Tamanho da fonte do título (opcional)
    title_x=0.5,  # Posição horizontal do título (0.5 é centralizado)
    title_y=0.9,  # Posição vertical do título (0.9 fica acima do gráfico)
    # Você pode ajustar a posição e o tamanho do título conforme necessário
...
fig5.update_layout(
    title='Quantidades de Cargas Canceladas (TOP 10)',  # Título do gráfico
    title_font_size=20,  # Tamanho da fonte do título (opcional)
    title_x=0.5,  # Posição horizontal do título (0.5 é centralizado)
    title_y=0.9,  # Posição vertical do título (0.9 fica acima do gráfico)
    # Você pode ajustar a posição e o tamanho do título conforme necessário
...
PCs_cancelado_cliente_card1 = df[df['STATUS'] == 'CANCELADO - CLIENTE'].shape[0]
...
PCs_cadastradas_card6 = df[df['SITUAC'] == 'D'].shape[0]

#######################################################################################################################################
# Card 7

data_de_hoje = datetime.today().strftime("%Y-%m-%d")
PCs_em_atraso_card7 = df[df['DATRET'] >= data_de_hoje].shape[0]
    ...
    dbc.Row([
        dbc.Col(
            dbc.Card(
                ...
                style={"max-width": "10rem", "width": "11rem", "background-color": "#003B34", "border-radius": "10px"}
            ),
        ...
        dbc.Col(
            dbc.Card(
                ...
                className="text-white",
                style={"max-width": "10rem", "width": "11rem", "background-color": "#003B34", "border-radius": "10px"}  # Adicionado estilo fixo e bordas arredondadas
            ),
        ),
        dbc.Col(
            dbc.Card(
        ...
        dbc.Col(
            dbc.Card(
                ...
                children=[
                    dbc.CardHeader("Cargas Cadastradas", style={"font-size": "0.9rem", "font-weight": "bold", "text-align": "center"}),
                    dbc.CardBody([
...
def update_card1(value_data, value_linhas, value_clientes, value_fil, value_ponto_ini, value_ponto_fim):
    ...
    if triggered_input in ['dropdown_data','lista_linhas', 'select_clientes', 'select_fil', 'select_ponto_ini', 'select_ponto_fim']:
        ...
        PCs_cancelado_cliente_card1 = tabela_filtrada_final[tabela_filtrada_final['STATUS'] == 'CANCELADO - CLIENTE'].shape[0]
...
def update_card2(value_data, value_linhas, value_clientes, value_fil, value_ponto_ini, value_ponto_fim):
    ...
    if triggered_input in ['dropdown_data', 'lista_linhas', 'select_clientes', 'select_fil', 'select_ponto_ini', 'select_ponto_fim']:
        ...
        PCs_declinadas_cliente_card2 = tabela_filtrada_final[tabela_filtrada_final['STATUS'] == 'CANCELADO - CLIENTE'].shape[0]
...
def update_card6(value_data, value_linhas, value_clientes, value_fil, value_ponto_ini, value_ponto_fim):
    ...
    if triggered_input in ['dropdown_data', 'lista_linhas', 'select_clientes', 'select_fil', 'select_ponto_ini', 'select_ponto_fim']:
        ...
        PCs_cadastradas_card6 = tabela_filtrada_final[tabela_filtrada_final['SITUAC'] == 'D'].shape[0]
...
def update_card7(value_data, value_linhas, value_clientes, value_fil, value_ponto_ini, value_ponto_fim):
    ...
    if triggered_input in ['dropdown_data', 'lista_linhas', 'select_clientes', 'select_fil', 'select_ponto_ini', 'select_ponto_fim']:
        ...
        data_de_hoje = datetime.today().strftime("%Y-%m-%d")
        PCs_em_atraso_card7 = tabela_filtrada_final[tabela_filtrada_final['DATRET'] >= data_de_hoje].shape[0]
...
def update_graf3(value_data, value_fil, value_ponto_ini, value_ponto_fim):
    ctx = dash.callback_context
    ...
    if triggered_input in ['dropdown_data', 'lista_linhas', 'select_fil', 'select_ponto_ini', 'select_ponto_fim']:
        ...
        status_desejados_graf3 = ["CARGA RECUSADA", "CANCELADO - CLIENTE", "DECLINADO - FALTA ATENDIMENTO"]
        tabela_filtrada_final_filtrado_graf3 = tabela_filtrada_final[tabela_filtrada_final['STATUS'].isin(status_desejados_graf3)]

        # Agrupar por TOMADOR e contar a ocorrência de cada STATUS
        contagem_por_tomador_graf3 = tabela_filtrada_final_filtrado_graf3.groupby('TOMADOR')['STATUS'].value_counts().unstack(fill_value=0)
...
def update_graf4(value_data,value_clientes):
    ctx = dash.callback_context
    ...
    if triggered_input in ['dropdown_data', 'select_clientes']:
        ...
        tabela_filtrada_final_fatiada = tabela_filtrada_clientes[
            tabela_filtrada_clientes.index.isin(tabela_filtrada_data.index) 
        ]
...
def update_graf5(value_data, value_clientes):
    ctx = dash.callback_context
    ...
    if triggered_input in ['dropdown_data', 'select_clientes']:
        ...
        status_desejados_graf5 = ["CARGA RECUSADA", "CANCELADO - CLIENTE", "DECLINADO - FALTA ATENDIMENTO"]
        df_filtrado_graf5 = tabela_filtrada_final[tabela_filtrada_final['STATUS'].isin(status_desejados_graf5)]
        ...
        # Ordenar os tomadores pelo total em ordem decrescente e pegar os top 5
        top_5_tomadores_graf5 = contagem_por_tomador_graf5.sort_values(by='TOTAL', ascending=False).head(10)
        ...
        fig5.update_layout(
            title='Quantidade de Cargas Cancelada (TOP 10)',  # Título do gráfico
            title_font_size=20,  # Tamanho da fonte do título (opcional)
            title_x=0.5,  # Posição horizontal do título (0.5 é centralizado)
            title_y=0.9,  # Posição vertical do título (0.9 fica acima do gráfico)
            # Você pode ajustar a posição e o tamanho do título conforme necessário
