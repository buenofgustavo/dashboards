...
quantidades_CODLINLIN = [
    df[(df['SITUAC'] != 'C') & df['CTE'].notnull()].shape[0],
    df[(df['SITUAC'] != 'C') & df['PV'].isnull()].shape[0],
    df[(df['SITUAC'] != 'C') & df['CTE'].isnull() & df['PV'].notnull()].shape[0],
    df[df['CODMTC'] == 22].shape[0],
    df[df['CODMTC'] == 21].shape[0],
    df[df['CODMTC'] == 23].shape[0]
]

cores_personalizadas = ['#4285F4', '#A4C3F5', '#17cfcf', '#FFF080', '#D32F2F', '#FFB366']

fig = px.pie(names=situacoes_CODLINLIN,
             values=quantidades_CODLINLIN, 
...
filtered_df = df[(df['DATRET'].notnull()) & (df['DATRET'] < data_ontem) & ~df['SITUAC'].isin(['C']) & (df['PV'].isnull())]
    ...
    dbc.Row([
        dbc.Col(
            dbc.Card(
                ...
                style={"max-width": "8rem", "width": "11rem", "background-color": "#A4C3F5", "border-radius": "10px"}
            ),
        ...
        dbc.Col(
            dbc.Card(
                ...
                className="text-white",
                style={"max-width": "8rem", "width": "11rem", "background-color": "#A4C3F5", "border-radius": "10px"}  # Adicionado estilo fixo e bordas arredondadas
            ),
        ),
        ...
        dbc.Col(
            dbc.Card(
                ...
                className="text-white",
                style={"max-width": "7.5rem", "width": "11rem", "background-color": "#A4C3F5", "border-radius": "10px"}  # Adicionado estilo fixo e bordas arredondadas
            ),
        ),
        dbc.Col(
            dbc.Card(
...
def update_output(value_data, value_linhas, value_clientes, value_fil, value_ponto_ini, value_ponto_fim):
    ...
    if triggered_input in ['dropdown_data', 'lista_linhas', 'select_clientes', 'select_fil', 'select_ponto_ini', 'select_ponto_fim']:
        ...
        quantidades_CODLINLIN = [
            tabela_filtrada_final[(tabela_filtrada_final['SITUAC'] != 'C') & tabela_filtrada_final['CTE'].notnull()].shape[0],
            tabela_filtrada_final[(tabela_filtrada_final['SITUAC'] != 'C') & tabela_filtrada_final['PV'].isnull()].shape[0],
            tabela_filtrada_final[(tabela_filtrada_final['SITUAC'] != 'C') & tabela_filtrada_final['CTE'].isnull() & df['PV'].notnull()].shape[0],
...
def update_card7(value_data, value_linhas, value_clientes, value_fil, value_ponto_ini, value_ponto_fim):
    ...
    if triggered_input in ['dropdown_data', 'lista_linhas', 'select_clientes', 'select_fil', 'select_ponto_ini', 'select_ponto_fim']:
        ...
        data_ontem = data_hoje - timedelta(days=1)
        # Filtrando os dados conforme as condições da sua consulta SQL
        filtered_df = tabela_filtrada_final[(tabela_filtrada_final['DATRET'].notnull()) & (tabela_filtrada_final['DATRET'] < data_ontem) & ~tabela_filtrada_final['SITUAC'].isin(['C', 'B']) & (tabela_filtrada_final['PV'].isnull())]
...
def update_card7(value_data, value_linhas, value_clientes, value_fil, value_ponto_ini, value_ponto_fim):
    ...
    if triggered_input in ['dropdown_data', 'lista_linhas', 'select_clientes', 'select_fil', 'select_ponto_ini', 'select_ponto_fim']:
        ...
        tabela_filtrada_final['TOTALFRETE'] = tabela_filtrada_final['TOTALFRETE'].astype(float)
        
        df_filtrado_card8 = tabela_filtrada_final[(tabela_filtrada_final['TOTALFRETE'].notnull()) & (tabela_filtrada_final['SITUAC'].isin(['D', 'M', 'B']))]
...
def update_graf6(value_data, value_linhas, value_clientes, value_fil, value_ponto_ini, value_ponto_fim):
    ctx = dash.callback_context
